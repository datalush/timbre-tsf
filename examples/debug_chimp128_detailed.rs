...
fn generate_iot_temperature(n: usize, stability: f64) -> Vec<f64> {
    ...
    let possible_temps: Vec<f64> = (180..=250).map(|t| t as f64 / 10.0).collect();
...
fn main() {
    ...
    println!(
        "🔁 Valores idénticos consecutivos: {} de {} ({:.2}%)",
        identical_count,
        data.len() - 1,
        identical_pct
    );
    ...
    println!(
        "🔢 Valores bit-a-bit idénticos: {} de {} ({:.2}%)",
        bitwise_identical,
        data.len() - 1,
    ...
    println!(
        "  XOR = 0 (idénticos):     {} ({:.1}%)",
        xor_zero,
        xor_zero as f64 / 10.0
    );
    println!(
        "  XOR pequeño (<10 bits):  {} ({:.1}%)",
        xor_small,
        xor_small as f64 / 10.0
    );
    println!(
        "  XOR grande (>=10 bits):  {} ({:.1}%)",
        xor_large,
        xor_large as f64 / 10.0
    );
    ...
    for i in 0..30 {
        ...
        let is_same = if i > 0 && data[i] == data[i - 1] {
            "✓ IGUAL"
        } else {
            ""
        };
    ...
    println!(
        "  {} idénticos × 1 bit:   {} bits",
        identical_count, identical_values_bits
    );
    println!(
        "  {} diferentes × ~12 bits: {} bits",
        different_values, different_values_bits
    );
    println!(
        "  Total teórico:        {} bits = {} bytes",
        theoretical_bits, theoretical_bytes
    );
    println!("  Bits/valor teórico:   {:.2}", theoretical_bits_per_value);

    println!("\n⚠️  Comparación:");
    println!(
        "  Teórico:  {:.2} bits/valor ({} bytes)",
        theoretical_bits_per_value, theoretical_bytes
    );
    println!(
        "  Real:     {:.2} bits/valor ({} bytes)",
        bits_per_value,
        encoded.len()
    );
    println!(
        "  Overhead: {:.2}x ({} bytes extra)",
