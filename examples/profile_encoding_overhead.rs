...
fn profile_arrow_conversion_only(batch: &RecordBatch) -> std::time::Duration {
    ...
    for (device_id, rows) in rows_by_device {
        let schemas = vec![
            MeasurementSchema::new("temperature", TSDataType::Float, TSEncoding::Chimp128, CompressionType::Snappy),
            MeasurementSchema::new("humidity", TSDataType::Float, TSEncoding::Chimp128, CompressionType::Snappy),
            MeasurementSchema::new("pressure", TSDataType::Float, TSEncoding::Chimp128, CompressionType::Snappy),
        ];
    ...
    println!("    (extraction: {:.1}%, grouping: {:.1}%, tablets: {:.1}%)",
...
fn profile_encoding_only(encoding: TSEncoding, compression: CompressionType) -> std::time::Duration {
    println!("\n=== Profiling Tablet → TsFile Writing (encoding={:?}, compression={:?}) ===", encoding, compression);
    ...
    for device in &devices {
        ...
        let mut tablet = Tablet::new(*device, schemas, vec![ColumnCategory::Field; 3], rows_per_device);
    ...
    // Register schemas
    for device in &devices {
        writer.register_timeseries(*device, MeasurementSchema::new("temperature", TSDataType::Float, encoding, compression)).unwrap();
        writer.register_timeseries(*device, MeasurementSchema::new("humidity", TSDataType::Float, encoding, compression)).unwrap();
        writer.register_timeseries(*device, MeasurementSchema::new("pressure", TSDataType::Float, encoding, compression)).unwrap();
    }
    ...
    println!("  Throughput: {:.2} MB/s", (file_size as f64 / 1_000_000.0) / total.as_secs_f64());
...
fn main() {
    ...
    println!("Test dataset: {} rows (5 devices × 400K rows each)\n", num_rows);
    ...
    let encoding_nocomp = profile_encoding_only(TSEncoding::Chimp128, CompressionType::Uncompressed);
    ...
    println!("Arrow → Tablet conversion: {:?} ({:.1}% of full pipeline)",
        conversion_time, 100.0 * conversion_time.as_secs_f64() / full_time.as_secs_f64());

    println!("\nEncoding overhead (Tablet → TsFile):");
    println!("  Chimp128 + Snappy: {:?} ({:.1}% of full pipeline)",
        encoding_chimp, 100.0 * encoding_chimp.as_secs_f64() / full_time.as_secs_f64());
    println!("  Plain + Snappy:    {:?} ({}x faster than Chimp128)",
        encoding_plain, encoding_chimp.as_secs_f64() / encoding_plain.as_secs_f64());
    println!("  Chimp128 + No compression: {:?} (compression overhead: {:?})",
        encoding_nocomp, encoding_chimp - encoding_nocomp);
    ...
    if encoding_percentage > 60.0 {
        println!("✅ BOTTLENECK CONFIRMED: Encoding is {:.1}% of total time", encoding_percentage);
        println!("   Arrow conversion is only {:.1}% - already well optimized!", conversion_percentage);
        ...
        println!("⚠️  UNEXPECTED: Encoding is only {:.1}% of total time", encoding_percentage);
