...
fn main() {
    ...
    for i in 0..iterations {
        ...
        if i == 0 {
            println!("Iteration {}: {:>7.2} ms (init: {:>6.2}, write: {:>6.2}, finish: {:>6.2})",
                i + 1,
    ...
    let avg_phase1 = times.iter().map(|(p1, _, _, _)| p1.as_secs_f64()).sum::<f64>() / times.len() as f64 * 1000.0;
    let avg_phase2 = times.iter().map(|(_, p2, _, _)| p2.as_secs_f64()).sum::<f64>() / times.len() as f64 * 1000.0;
    let avg_phase3 = times.iter().map(|(_, _, p3, _)| p3.as_secs_f64()).sum::<f64>() / times.len() as f64 * 1000.0;
    let avg_total = times.iter().map(|(_, _, _, t)| t.as_secs_f64()).sum::<f64>() / times.len() as f64 * 1000.0;

    let min_total = times.iter().map(|(_, _, _, t)| t.as_secs_f64()).min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() * 1000.0;
    let max_total = times.iter().map(|(_, _, _, t)| t.as_secs_f64()).max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() * 1000.0;

    println!("\n=== PROFILING RESULTS ({} iterations, 100K rows) ===", iterations);
    println!("Phase 1 (Converter init):  {:>8.2} ms ({:>5.1}%)",
        avg_phase1,
        avg_phase1 / avg_total * 100.0);
    println!("Phase 2 (write_batch):     {:>8.2} ms ({:>5.1}%)",
        avg_phase2,
        avg_phase2 / avg_total * 100.0);
    println!("Phase 3 (finish):          {:>8.2} ms ({:>5.1}%)",
        avg_phase3,
    ...
    println!("Gap to close:              {:>8.2} ms ({:>5.1}%)",
        avg_total - 14.99,
    ...
    if avg_phase2 / avg_total > 0.7 {
        println!("HOT PATH: write_batch() is the dominant bottleneck ({:.1}%)", avg_phase2 / avg_total * 100.0);
        println!("  - Focus on: extract_column_bulk, tablet operations, device grouping");
    }
    if avg_phase3 / avg_total > 0.2 {
        println!("SECONDARY: finish() takes {:.1}% of time", avg_phase3 / avg_total * 100.0);
