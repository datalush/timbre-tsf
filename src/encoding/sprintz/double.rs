...
/// Double Sprintz Encoder
pub struct DoubleSprintzEncoder {
    values: Vec<f64>,
    byte_cache: Vec<u8>,
    group_num: usize,
...
impl DoubleSprintzEncoder {
    ...
    pub fn encode(&mut self, value: f64, out: &mut Vec<u8>) -> Result<()> {
    ...
    fn predict(&mut self, value: f64, prev_value: f64) -> i64 {
        let curr_bits = value.to_bits() as i64;
        let prev_bits = prev_value.to_bits() as i64;
        ...
        zigzag_encode_i64(raw_pred)
    }

    fn bit_pack(&mut self, pre_value: f64, convert_buffer: &[i64]) -> Result<()> {
        let bit_width = get_max_bit_width_i64(convert_buffer);

        let mut pack_buf = Vec::new();
        pack_8values_i64(convert_buffer, bit_width, &mut pack_buf);

        self.byte_cache.push(bit_width);
        self.byte_cache.write_f64::<LittleEndian>(pre_value)?;
...
impl Default for DoubleSprintzEncoder {
    fn default() -> Self {
        Self::new()
    }
}

/// Double Sprintz Decoder
pub struct DoubleSprintzDecoder {
...
impl DoubleSprintzDecoder {
    ...
    pub fn read_double(&mut self, input: &[u8], pos: &mut usize) -> Result<f64> {
        ...
        *pos += 1;
        ...
        if (bit_width & (1 << 7)) != 0 {
            ...
            for i in 0..size {
                ...
                self.current_buffer[i] = cursor.read_f64::<LittleEndian>()?;
                *pos += 8;
            }
        } else {
            ...
            let mut cursor = Cursor::new(&input[*pos..*pos + 8]);
            let pre_bits = cursor.read_f64::<LittleEndian>()?;
            *pos += 8;
    ...
    fn recalculate(&mut self, mut convert_buffer: Vec<i64>) -> Result<()> {
        ...
        match self.predict_method {
            ...
            PredictMethod::Fire => {
                ...
                for (i, &err) in convert_buffer.iter().enumerate().take(BLOCK_SIZE) {
                    let pred = self.fire_pred.predict(prev_bits);
...
impl Default for DoubleSprintzDecoder {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    #[test]
    fn test_double_single_value() {
        let mut encoder = DoubleSprintzEncoder::new();
        let mut out = Vec::new();

        encoder.encode(f64::MAX, &mut out).unwrap();
        encoder.flush(&mut out).unwrap();

        let mut decoder = DoubleSprintzDecoder::new();
        let mut pos = 0;
        let val = decoder.read_double(&out, &mut pos).unwrap();
        assert_eq!(val, f64::MAX);
    }

    #[test]
    fn test_double_increasing() {
        let mut encoder = DoubleSprintzEncoder::new();
        let mut out = Vec::new();

        let start = 7.101f64;
        for i in 0..100 {
            encoder.encode(start + 2.0 * i as f64, &mut out).unwrap();
        }
        encoder.flush(&mut out).unwrap();
        ...
        for i in 0..100 {
            let val = decoder.read_double(&out, &mut pos).unwrap();
            assert!((val - (start + 2.0 * i as f64)).abs() < 0.000001);
        }
    }

    #[test]
    fn test_double_special_values() {
        let values = vec![f64::MIN, f64::MAX, -0.0f64, 0.0f64, f64::NAN];
        let mut encoder = DoubleSprintzEncoder::new();
        let mut out = Vec::new();
        ...
        for &expected in &values {
            let actual = decoder.read_double(&out, &mut pos).unwrap();
            if expected.is_nan() {
