...
use timbre_tsf::common::statistic::{
    BooleanStatistic, DoubleStatistic, FloatStatistic, Int32Statistic, Int64Statistic,
    StatisticEnum, StringStatistic,
};
...
fn test_statistic_enum_size() {
    ...
    println!(
        "  BooleanStatistic: {} bytes",
        size_of::<BooleanStatistic>()
    );
    ...
    assert!(
        enum_size <= 128,
        "StatisticEnum should not be excessively large"
    );
...
fn test_vs_box_overhead() {
    ...
    println!(
        "  Box<dyn Statistic>:    {} bytes (pointer) + heap allocation",
        box_size
    );
